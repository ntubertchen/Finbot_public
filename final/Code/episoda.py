...
def initializer(episode1):
    ...
    if episode1.goal == 'exchange':
        ...
        init_temp = ["Please give me the share price detail of "+episode1.inform_slots['stock_name']+" "+str(episode1.inform_slots['date']), "What is the close price of "+episode1.inform_slots['stock_name']+" on "+str(episode1.inform_slots['date'])+"?", "I'd like to know the open and low price of "+episode1.inform_slots['stock_name']+" on "+str(episode1.inform_slots['date'])]
        ...
        init_temp = ["what's the exchange rate of "+episode1.inform_slots['action']+" "+episode1.inform_slots['money_name']+" by TWD now in bank with "+episode1.inform_slots['types']+"?", "I want to "+episode1.inform_slots['action']+" some "+episode1.inform_slots['money_name']+" with "+episode1.inform_slots['types']]
        ...
        return random.choice(init_temp)
def initializer2(episode1):
    init_temp = []
    if episode1.goal == 'exchange':
        #init_temp = ["Please give me the exchange rate of "+episode1.inform_slots['country1']+" to "+episode1.inform_slots['country2'],"I'd like to know the exchange rate of "+episode1.inform_slots['country1']+" to "+episode1.inform_slots['country2']]
        init_temp.append(str("What is the exchange rate of "+episode1.inform_slots['country2']+"?"))
        init_temp.append(str("How's the exchange rate of "+episode1.inform_slots['country2']+"?"))
        init_temp.append(str("Give me the exchange rate"))
        init_temp.append(str("I'd like to know the exchange rate"))
...
class episode(object):
    ...
    def __init__(self, arg):
        ...
        self.mid_date = self.date_gen(None,3).toordinal()
        self.comp_ind = None
        self.inform_slots = self.slots_fill_in(self.mid_date)

    def getgoal(self):
        return episode.user_goal[random.randint(0,len(episode.user_goal)-1)]

    def slots_fill_in(self,mid_date):
        tmp = {}
        tmplist = list(episode.slots[self.goal]) #list of dictionry keys
        for slot in tmplist:
            if slot in ['date','time_end','time_start']:
                tmp[slot] = str(self.date_gen(self.mid_date,episode.time_mode[slot]))
            else:
    ...
    def date_gen(self,mid_date,mode):
        if mode==3:
            a = date.today().replace(day=1, month=1).toordinal()+1
            b = date(2017,3,31).toordinal()-1
            return date.fromordinal(random.randint(a,b))
        start_date = self.mid_date+1 if mode==2 else date.today().replace(day=1, month=1).toordinal() 
        end_date = self.mid_date-1 if mode==1 else date(2017,3,31).toordinal()
...
# need real data 
    ...
    time_mode = {"date":0,"time_start":1,"time_end":2}
