...
def run_an_episode():
	...
	while (not over):
		...
		prompt = "enter user intent:[query,USDX,exchange,get_exchange_rate]\n->"
		a = input(prompt)
		while a not in["query","USDX","exchange","get_exchange_rate"]:
			a = input(prompt)
		frame_output['action_item'] = [a]
...
if __name__ == '__main__':
	...
	for _ in range(30):
		...
		print('reward: {0} ,it was a {1} dialogue. frame_accuray:{2}'.format(r,'successful' if s is True else 'failing',1 if s is True else (turn/2)-1/(turn/2)))
